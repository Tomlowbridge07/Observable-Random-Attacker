...
\node[font=\sffamily\large] (OLabel) [shift={(0.5,1.5)}] at (35) {Observation, $o_{j}$};
\node[font=\sffamily\large] (SLabel) [shift={(-1.5,0.5)}] at (02) {\rotatebox{90}{Time since last visit, $s_{j}$}};

\node[main node] (Example) [shift={(-1,1)}] at (05) {$(s_{j},o_{j})$};
...
\node[font=\sffamily\large] (OLabel) [shift={(0.5,1.5)}] at (23) {Observation, $o_{j}$};
\node[font=\sffamily\large] (SLabel) [shift={(-1.5,0.5)}] at (01) {\rotatebox{90}{Time since last visit, $s_{j}$}};

\node[main node] (Example) [shift={(-1,1)}] at (03) {$(s_{j},o_{j})$};
...
\node[font=\sffamily\large] (OLabel) [shift={(0.5,1.5)}] at (21) {Observation, $o_{j}$};
\node[font=\sffamily\large] (SLabel) [shift={(-2.4,0.5)}] at (00) {\rotatebox{90}{Time since last visit, $s_{j}$}};

\node[main node] (Example) [shift={(-1,1)}] at (01) {$(s_{j},o_{j})$};
...
\begin{example}
Suppose $X=2.1$ and $b=5$, then our state space is reduced to $\Omega= \{ (3,o) \, | o=0,1,...,5 \} \cup \{(4,0) \}$ and we make the decision at state $(3,o)$ first based on the calculation of $c(2+1)(\lambda(2-2.1+1)+o)=c(2.7\lambda+3o)$ versus $c(\lambda (2.1+1)-o)=c(3.1\lambda -o)$ so depends if $0.1 \lambda \geq o$ or $0.1 \lambda > o $.

Suppose now that we have $o=1$ and $\lambda=20$ then we fall into the first case and we will make the decision based on the cost, $\omega$:
\begin{itemize}
...
However if instead $o=4$ while $\lambda=20$ then we fall into the first case and will make the decision based on the cost, $\omega$:
\begin{itemize}
