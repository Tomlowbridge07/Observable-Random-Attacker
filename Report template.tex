...
%We will now look at the special case that $v_{max}=0$, that is $\lambda (1-R)$ (Which is guaranteed by $\lambda \leq 1$) so then we will we will always renew now, or we will inccur a higher $g$ than the neglecting strategy. So to get $ 0\leq g \leq c \lambda$ we will always renew when we hit the state $(B,v) \, \forall v$. We know that this will give an average cost of $g=\frac{\text{Expected cost per renewal}}{B}=$
