...
This brings about a reasoning to introduce $\max\limits_{0 \leq g \leq c \lambda} v_{\text{crit}}=\ceil{\frac{c \lambda}{c}+\lambda(R-1)}=\ceil{R}$. Now as  $0 \leq R < 1$ we will get the furtherest $v_{crit}$ is $0$ or $1$ (if $R=0$ or $R \neq 0$ respectively).

\begin{note}
These are only the extreme cases where $g= c \lambda$, it is possible that we do not get a $v_{crit}$ at all, i.e we will always renew.
\end{note} 

\textbf{Case 1: $R=0$}

When $R=0$ then at worst under this non-neglecting case we will never wait for any $v=0$ (as the max crit is $v_{\text{crit}}=0$). So this will generate an average cost of

\begin{align*}
g(\omega)=\frac{\omega}{\floor{B}+1}
\end{align*}

So as we will need to be in the case with $g \leq c \lambda$ for this to take effect we will need $\omega \leq c \lambda (\floor{B}+1)$.

\textbf{Case 2: $R \neq 0$}

Then we have the possibility to wait in state $v=0$ and renew one step later, however this will only occur in the region where $v_{crit}=1$ that is that  $c \lambda (1-R) < g \leq c (1+ \lambda(1-R))$ (if we have $g \leq c \lambda (1-R)$ then we will still get $v_{crit}=0$)

\textbf{Subcase 1: $g \leq c \lambda (1-R)$}

If we fall into here, $v_{crit}=0$ so we still get that the average cost is
$g(\omega)=\frac{\omega}{\floor{B}+1}$ and so we need $g \leq c \lambda (1-R)$ meaning $\omega \leq c \lambda (1-R)(\floor{B}+1)$

We notice that $g \leq c \lambda (1-R) < c \lambda$ so we have no worries with being in this case.

\textbf{Subcase 2: $c \lambda (1-R) < g \leq c (1+ \lambda(1-R))$}

If we fall into here, $v_{crit}=1$ so we will get a slightly different average cost

\begin{align*}
g(\omega)&=\frac{\omega+c\lambda(1-R)}{\floor{B}+2} P(TPo(\lambda)=0) + \frac{\omega}{\floor{B}+1} P(TPo(\lambda) \neq 0) \\
&=\frac{\omega+c\lambda(1-R)}{\floor{B}+2} e^{-\lambda} + \frac{\omega}{\floor{B}+1} (1-e^{-\lambda}) \\
&=\frac{(\floor{B}+1)(\omega+c(\lambda(1-R))e^{-\lambda} + (\floor{B}+2)(\omega) (1-e^{-\lambda})}{(\floor{B}+2)(\floor{B}+1)} \\
&=\frac{(\floor{B}+1)(c(\lambda(1-R))e^{-\lambda} + (\floor{B}+2-e^{-\lambda})\omega}{(\floor{B}+2)(\floor{B}+1)}
\end{align*}

and we need that $c \lambda (1-R) < g \leq c (1+ \lambda(1-R))$ so we need that $c \lambda (\floor{B}+1)(1-R) \leq \omega \leq \frac{c(\floor{B}+1)(\floor{B}+2)}{\floor{B}+2-e^{-\lambda}}+c \lambda (\floor{B}+1)(1-R) $

We will still require that $g \leq c \lambda$, so we will need to limit ourselves to $\lambda R \geq 1$ otherwise the upper limit on $g$ becomes $c \lambda$ (translating to $\frac{c \lambda (\floor{B}+1)(\floor{B}+2-(1-R)e^{-\lambda})}{\floor{B}+2-e^{-\lambda}} $ in the $\omega$ limit)

\subsection{Conclusion on deterministic attack time}
We conclude our results on determinsitic attack time by a case, by case basis:

\begin{itemize}
\item if $R=0$ then:
\begin{itemize}
\item if $\omega \leq c \lambda (\floor{B}+1)=c \lambda (B+1)= c \lambda
(E[X]+1))$ then we renew immediately in all states
\item if $\omega > c \lambda (E[X]+1)$ then we never renew
\end{itemize}
\item if $R \neq 0$ then:
\begin{itemize}
\item if $\omega \leq c \lambda (1-R)(\floor{B}+1)$ then we renew immediately in all states
\item if $\lambda R \geq 1$ then:
\begin{itemize}
\item if $c \lambda (\floor{B}+1)(1-R) \leq \omega \leq \frac{c(\floor{B}+1)(\floor{B}+2)}{\floor{B}+2-e^{-\lambda}}+c \lambda (\floor{B}+1)(1-R)$ then we renew immediately in all states with $v \geq 1$ but we wait one period before renewing in the state $(\floor{B}+1,0)$
\item if $\omega > \frac{c(\floor{B}+1)(\floor{B}+2)}{\floor{B}+2-e^{-\lambda}}+c \lambda (\floor{B}+1)(1-R)$ then we never renew
\end{itemize}
\item if $\lambda R < 1$ then:
\begin{itemize}
\item if $c \lambda (\floor{B}+1)(1-R) \leq \omega \leq \frac{c \lambda (\floor{B}+1)(\floor{B}+2-(1-R)e^{-\lambda})}{\floor{B}+2-e^{-\lambda}} $ then we renew immediately in all states with $v \geq 1$ but we wait one period before renewing in the state $(\floor{B}+1,0)$
\item if $\omega > (\floor{B}+1)(\floor{B}+2-(1-R)e^{-\lambda}){\floor{B}+2-e^{-\lambda}} $ then we never renew
\end{itemize}
\end{itemize}
\end{itemize}

For a given scenario (i.e a known $R$ and $\lambda$) we can determine our case and hence our index. We also note that these decision costs are $non-decreasing$.
